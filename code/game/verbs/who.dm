		...
		for(var/client/C in GLOB.clients)
			...
			if(C.mob) //Juuuust in case
				...
				if(isobserver(C.mob))
					...
					if(C.admin_holder?.rights & (R_MOD|R_COUNCIL))
...
/client/verb/staffwho()
	...
	if(CONFIG_GET(flag/show_devs))
		LAZYSET(mappings, "<B style='color:blue'>Coders</B>", R_PROFILER)
	LAZYSET(mappings, "<B style='color:#e3a759'>Game Masters</B>", R_GAMEMASTER)
	LAZYSET(mappings, "<B style='color:red'>Admins</B>", R_ADMIN)
	LAZYSET(mappings, "<B style='color:#42b898'>Whitelist Maintainers</B>", R_WHITELISTM)
