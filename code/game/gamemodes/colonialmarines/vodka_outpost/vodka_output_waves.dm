...
/datum/game_mode/vodka_outpost/proc/spawn_vodka_outpost_xenos(datum/vodka_outpost_wave/wave_data)
	...
	while(xenobots_to_spawn-- > 0)
		xenobot_pool += pick(wave_data.wave_castes_bots) // Adds the wave's xenos to the current pool



/datum/game_mode/vodka_outpost/attempt_to_join_as_xeno(mob/xeno_candidate, instant_join = 0)
	...
	if(RoleAuthority.castes_by_name[userInput])
		...
		var/spawnbot_loc = pick(xenobot_spawns)
