...
/datum/admins/proc/associate(client/C)
	if(istype(C))
		...
		if(owner.admin_holder.rights & R_WHITELISTM)
			if(!world.GetConfig("admin", C.ckey))
				world.SetConfig("APP/admin", C.ckey, "role = Whitelist Maintainer")
		if(owner.admin_holder.rights & R_GAMEMASTER)
			if(!world.GetConfig("admin", C.ckey))
				world.SetConfig("APP/admin", C.ckey, "role = Game Master")
